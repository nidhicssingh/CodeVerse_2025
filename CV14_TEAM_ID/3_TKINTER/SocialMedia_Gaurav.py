...
# Initialize Main App
app = tk.Tk()
app.title("Gaurav's PROFILE")
app.geometry("800x600")
app.configure(bg="#CAA1C1")

# Style
style = ttk.Style()
style.configure("TButton", padding=6, relief="flat", background="#D2B6CC", font=("Snap ITC", 12),foreground="white")

# Frames
frame_top = tk.Frame(app, bg="#cf65b1", height=50)
frame_top.pack(fill=tk.X)

frame_content = tk.Frame(app, bg="#CE94BC")
frame_content.pack(fill=tk.BOTH, expand=True)

# Title
title = tk.Label(frame_top, text="Gaurav'S PROFILE", fg="white", bg="#CF6FB8", font=("Snap ITC", 22, "bold"))
title.pack(pady=5)

# Post Frame
post_frame = tk.LabelFrame(frame_content, text="Create Post", padx=10, pady=10, bg="#eee8ed", font=("Snap ITC", 12, "bold"))
...
def view_profile():
    ...
    window.title("Profile: Gaurav")
    window.geometry("500x600")
    window.configure(bg="#fbc2eb")
    ...
    tk.Label(window, text="👤 Gaurav's Profile", font=("Arial", 16, "bold"), bg="#fbc2eb",fg="#8e44ad").pack(pady=5)
    tk.Button(window, text="✏️ Edit Profile", command=edit_profile, bg="#7D076B", fg="white").pack(pady=10)

    posts = [p for p in send_post.posts if p["user"] == "Gaurav"]

    for post in posts[::-1]:
        frame = tk.Frame(window, bg="#fff6f6", padx=10, pady=10)
        frame.pack(pady=5, fill=tk.X)
