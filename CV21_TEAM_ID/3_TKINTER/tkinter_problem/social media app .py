...
def view_profile():
    ...
    tk.Label(window, text="👤 Gange's Profile", font=("Arial", 16, "bold"), bg="white").pack(pady=10)
    tk.Button(window, text="✏️ Edit Profile", command=edit_profile, bg="#4CAF50", fg="white").pack(pady=5)

    # FIX: Filter posts by correct user
    posts = [p for p in send_post.posts if p["user"] == "Gange DM"]
    image_refs = []
