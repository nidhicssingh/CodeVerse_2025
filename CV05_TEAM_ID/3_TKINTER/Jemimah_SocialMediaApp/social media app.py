...
# Initialize Main App
app = tk.Tk()
app.title("JEMIMAH'S PROFILE")
app.geometry("800x600")
app.configure(bg="#aa3665")

# Style
style = ttk.Style()
style.configure("TButton", padding=6, relief="flat", background="#D34BA8", foreground="white",font=("Arial", 12,"italic"))

# Frames
frame_top = tk.Frame(app, bg="#1C8FA5", height=50)
frame_top.pack(fill=tk.X)

frame_content = tk.Frame(app, bg="#1C8FA5")
frame_content.pack(fill=tk.BOTH, expand=True)

# Title
title = tk.Label(frame_top, text="Jemimah's Profile", font=("Helvetica", 20, "italic","bold") , foreground="white", background="#3789C0")
title.pack(pady=10)

# Post Frame
post_frame = tk.LabelFrame(frame_content, text="Post here", padx=10, pady=10, bg="white" , font=("verdana",12,"bold"))
post_frame.pack(pady=10)

post_text = tk.Text(post_frame, height=4, width=60, font=("Georgia", 12),wrap="word",bd=2,relief="groove")
post_text.pack()
...
tk.Button(post_frame, text="Upload Image", command= upload_image,bg="#1C8FA5").pack(side=tk.LEFT, padx=5)
tk.Button(post_frame, text="Post", command=post,bg="#1C8FA5").pack(side=tk.RIGHT, padx=5)
...
def render_feed():
    ...
    for post in send_post.posts[::-1]:
        ...
        tk.Button(frame, text=f"❤️ {post['likes']}",bg="#1C8FA5", command=lambda p=post: [like_post.like(p), render_feed()]).pack(side=tk.LEFT)
        tk.Button(frame, text="Comment 📸",bg="#1C8FA5", command=lambda p=post: comment_post.comment_window(app, p)).pack(side=tk.LEFT, padx=5)
